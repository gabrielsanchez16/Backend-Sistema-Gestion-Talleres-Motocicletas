const {createServiceByWorkshop,deleteService,getAllServicesByOrder,updateService} = require('../controllers/serviceByWorkshop.controller');


const create = async (req, res) => {
    const {name_service,quantity_order,price_unit,id_order,id_service} = req.body;

    try {
        const service = await createServiceByWorkshop(name_service,quantity_order,price_unit,id_order,id_service);
        res.status(201).json({
